...
decNumber *decNumberFromString(decNumber *dn, const char chars[], decContext *set) {
    ...
    do {
        ...
        if (last == NULL) {
            ...
            if (*c != '\0') break;
            if (c >= firstexp + 10) {
                if (c > firstexp + 10 || *firstexp > '1') exponent = DECNUMMAXE * 2;
            }
...
decNumber *decNumberAdd(decNumber *res, const decNumber *lhs, const decNumber *rhs, decContext *set) {
    ...
    if (res == NULL || lhs == NULL || rhs == NULL || set == NULL) {
        if (res != NULL) decStatus(res, DEC_Invalid_operation, set);
        return res;
    }
...
decNumber *decNumberCompareTotalMag(decNumber *res, const decNumber *lhs,
    ...
    do {
        ...
        if (decNumberIsNegative(lhs)) {
            size_t needbytes = sizeof(decNumber) + (D2U(lhs->digits) - 1) * sizeof(Unit);
            a = (needbytes > sizeof(bufa)) ? (decNumber *)malloc(needbytes) : bufa;
            if (!a) {
                status |= DEC_Insufficient_storage;
                break;
            }
            if (a != bufa) allocbufa = a;
            decNumberCopy(a, lhs);
            a->bits &= ~DECNEG;
            lhs = a;
        }
        ...
        if (decNumberIsNegative(rhs)) {
            size_t needbytes = sizeof(decNumber) + (D2U(rhs->digits) - 1) * sizeof(Unit);
            b = (needbytes > sizeof(bufb)) ? (decNumber *)malloc(needbytes) : bufb;
            if (!b) {
                status |= DEC_Insufficient_storage;
                break;
            }
            if (b != bufb) allocbufb = b;
            decNumberCopy(b, rhs);
            b->bits &= ~DECNEG;
            rhs = b;
        }
...
decNumber *decNumberInvert(decNumber *res, const decNumber *rhs, decContext *set) {
    ...
    res->digits = decGetDigits(res->lsu, msuc - res->lsu + 1);
    res->exponent = 0;
    res->bits = 0;
    return res;
} /* decNumberInvert */
...
decNumber *decNumberLog10(decNumber *res, const decNumber *rhs, decContext *set) {
    ...
    if (!decCheckMath(rhs, set, &status)) do {
        ...
        if ((status & DEC_NaNs && !(status & DEC_sNaN)) || (a->bits & DECSPECIAL) || ISZERO(a)) {
            decNumberCopy(res, a);
            break;
        }
...
decNumber *decNumberNextToward(decNumber *res, const decNumber *lhs,
    ...
    if (result == BADINT) {
        decStatus(res, DEC_Insufficient_storage, set);
        return res;
    }
...
decNumber *decNumberRemainder(decNumber *res, const decNumber *lhs,
			       const decNumber *rhs, decContext *set) {
  uInt status = 0;
  if (res == NULL || lhs == NULL || rhs == NULL || set == NULL) return NULL;
            ...
            if (rotate != 0 && rotate != set->digits && !decNumberIsInfinite(res)) {
                ...
                if (shift > 0) {
                    ...
                    if (shift > msudigits) {
                        ...
                        *(msumax - 1) += (Unit)(rem * powers[DECDPUN - (shift - msudigits)]);
                    } else {
                        *msumax += (Unit)(save * powers[msudigits - shift]);
                    }
                }
                ...
                if (units > 0) {
                    ...
                    if (shift > 0) {
                        ...
                        *msumax += (Unit)(save * powers[msudigits]);
                    }
                ...
                if (shift > 0) {
                    if (shift == set->digits) {
                        *res->lsu = 0;
                        ...
                        Int extraDigits = res->digits + shift - set->digits;
                        if (extraDigits > 0) {
                            decDecap(res, extraDigits);
                        }
                    ...
                    if (rightShift >= res->digits) {
                        *res->lsu = 0;
                        res->digits = 1;
                    } else {
                        decShiftToLeast(res->lsu, D2U(res->digits), rightShift);
                        res->digits -= rightShift;
                    }
                }
            }
        }
...
enum decClass decNumberClass(const decNumber *dn, decContext *set) {
  if (dn == NULL || set == NULL) return DEC_CLASS_QNAN;
...
decNumber *decNumberCopy(decNumber *dest, const decNumber *src) {
    ...
    if (src->digits > DECDPUN) {
        const Unit *src_ptr = src->lsu + 1;
        const Unit *src_end = src->lsu + D2U(src->digits);
        Unit *dest_ptr = dest->lsu + 1;
...
#if DECCHECK
  if (decCheckOperands(res, DECUNUSED, rhs, DECUNCONT) != 0) {
    return res;
  }
#endif
...
#if DECDPUN == 1
    for (uInt i = 0; i < n && up >= dn->lsu; i++, ub++, up--) {
        *up = *ub;
    }
#else
    Int cut = MSUDIGITS(n);
...
static decNumber *decAddOp(decNumber *res, const decNumber *lhs,
    ...
    do {
        ...
        if ((maxdigits >= reqdigits) || (res == rhs && rhsshift > 0)) {
            ...
            acc = accbuff;
            if ((size_t)need * sizeof(Unit) > sizeof(accbuff)) {
...
static decNumber *decDivideOp(decNumber *res, const decNumber *lhs, const decNumber *rhs, decContext *set, Flag op, uInt *status) {
    ...
    do {
        ...
        if ((size_t)(acclength * sizeof(Unit)) > sizeof(accbuff)) {
        ...
        if ((size_t)((var1units + 1) * sizeof(Unit)) > sizeof(varbuff)) {
        ...
        if (!(op & DIVIDE)) {
            ...
            exponent += cut;
            var1initpad -= cut;
            for (Unit *u = msu1; cut >= DECDPUN; cut -= DECDPUN, u--) *u = 0;
        } else {
        ...
        if (op & DIVIDE) {
            ...
            if (op & (REMAINDER | REMNEAR)) {
                ...
                if (op & REMNEAR) {
                    ...
                    compare = decUnitCompare(accnext, tarunits, rhs->lsu, D2U(rhs->digits), rhs->exponent - exponent);
                    if (compare == BADINT) {
                        *status |= DEC_Insufficient_storage;
                        break;
                    }
                    ...
                    if (compare > 0 || (compare == 0 && wasodd)) {
                        ...
                        exprem = exp % DECDPUN;
                        accunits = -decUnitAddSub(accnext, accunits, rhs->lsu, D2U(rhs->digits), expunits, accnext, -(Int)powers[exprem]);
                        accdigits = decGetDigits(accnext, accunits);
                        accunits = D2U(accdigits);
                        bits ^= DECNEG;
                    }
    ...
    do {
        ...
        if (rhs->digits > NEEDTWO) {
            ...
            if (needbytes > (Int)sizeof(zlhibuff)) {
                ...
                if (alloclhi == NULL) { *status |= DEC_Insufficient_storage; break; }
                zlhi = alloclhi;
            }
            needbytes = irhs * (Int)sizeof(uInt);
            if (needbytes > (Int)sizeof(zrhibuff)) {
                allocrhi = (uInt *)malloc(needbytes);
                if (allocrhi == NULL) { *status |= DEC_Insufficient_storage; break; }
                zrhi = allocrhi;
            }
    ...
    if (h > 8) {
      ...
      *numone.lsu = 1;
      ...
      for (;;) {
        ...
        if (((a->digits + a->exponent) >= (t->digits + t->exponent + p + 1)) && (a->digits >= p))
          break;
...
decNumber *decCompareOp(decNumber *res, const decNumber *lhs,
    ...
    do {
        ...
        if (!set->extended) {
            if (lhs->digits > set->digits) {
                alloclhs = decRoundOperand(lhs, set, status);
                if (!alloclhs) { result = BADINT; break; }
                lhs = alloclhs;
            }
            if (rhs->digits > set->digits) {
                allocrhs = decRoundOperand(rhs, set, status);
                if (!allocrhs) { result = BADINT; break; }
                rhs = allocrhs;
            }
        }
#endif
        ...
        if (op == COMPTOTAL) {
            Int lhsNeg = decNumberIsNegative(lhs);
            Int rhsNeg = decNumberIsNegative(rhs);
            if (lhsNeg && !rhsNeg) { result = -1; break; }
            if (!lhsNeg && rhsNeg) { result = +1; break; }
        }
...
static Int decCompare(const decNumber *lhs, const decNumber *rhs, Flag abs) {
    ...
    Int compare = decUnitCompare(lhs->lsu, D2U(lhs->digits),
                                rhs->lsu, D2U(rhs->digits),
...
static Int decUnitCompare(const Unit *a, Int alength, const Unit *b, Int blength, Int exp) {
    ...
    if (exp == 0) {
        ...
        if (alength < blength) return -1;
        for (const Unit *l = a + alength - 1, *r = b + alength - 1; l >= a; l--, r--) {
            if (*l > *r) return 1;
            if (*l < *r) return -1;
        }
        return 0;
    ...
    need += 2;
    if ((size_t)need > SD2U(DECBUFFER*2+1)) {
        allocacc = (Unit *)malloc(need * sizeof(Unit));
...
static Int decUnitAddSub(const Unit *a, Int alength,
                         ...
                         Unit *c, Int m) {
    if (!a || !b || !c || alength < 1 || blength < 1) {
        return 0; // handle invalid input safely
    }

    const Unit *a_start = a;
    Unit *c_start = c;
    Unit *minC, *maxC;
    eInt carry = 0;
    Int add;
    ...
    if (bshift != 0) {
        ...
        if (a == c && bshift <= alength) {
            ...
            Unit *copy_end = c + bshift;
            for (; c < copy_end; a++, c++) {
                if (a < a_start + alength)
                    *c = *a;
                else
                    *c = 0;
            }
    ...
    // Main addition/subtraction loop for length overlap
    for (; c < minC; c++) {
        carry += (a < a_start + alength) ? *a++ : 0;
        carry += (b < b + blength) ? ((eInt)*b++) * m : 0;
    ...
    // Complete addition/subtraction for remaining units
    if (c < maxC) {
        for (; c < maxC; c++) {
            if (a < a_start + alength) {
                carry += *a++;
    ...
    if (carry == 0) return (Int)(c - c_start);
    if (carry > 0) {
        *c = (Unit)carry;
        c++;
        return (Int)(c - c_start);
    }

    add = 1;
    for (c = c_start; c < maxC; c++) {
        Int temp = DECDPUNMAX + add - *c;
        if (temp <= DECDPUNMAX) {
            *c = (Unit)temp;
            add = 0;
        } else {
...
static void decCopyFit(decNumber *dest, const decNumber *src,
    ...
    if (!dest || !src || !set || !residue || !status) return;
    dest->bits = src->bits;
    dest->exponent = src->exponent;
...
static uInt decCheckMath(const decNumber *rhs, decContext *set, uInt *status) {
    ...
    if (!ISZERO(rhs)) {
        int rhsDigits = rhs->digits;
        int rhsExp = rhs->exponent;
        int expPlusDigits = rhsExp + rhsDigits;
        if (rhsDigits > DEC_MAX_MATH ||
            expPlusDigits > (DEC_MAX_MATH + 1) ||
            expPlusDigits < 2 * (1 - DEC_MAX_MATH)) {
            *status |= DEC_Invalid_operation;
            return (*status != originalStatus);
        }
    }
...
#error GetInt may need updating [for Emin]
    ...
    if (ilength < 11) {
        ...
        if (ilength == 10) {
            Int maxval = neg ? 1999999997 : 999999999;
            Int checkpower = powers[got - DECDPUN];
            if (theInt / (Int)checkpower != (Int)*(up - 1)
                || (neg && theInt > maxval)
                || (!neg && theInt > maxval)) {
                theInt = save;
                ilength = 11;
...
static decNumber *decDecap(decNumber *dn, Int drop) {
    ...
    Int new_digits = dn->digits - drop;
    Unit *msu = dn->lsu + D2U(new_digits) - 1;
    Int cut = MSUDIGITS(new_digits);
...
static decNumber *decNaNs(decNumber *res, const decNumber *lhs,
    ...
    if (src->digits <= set->digits) {
        ...
        const Unit *ul = src->lsu;
        Unit *ur = res->lsu;
        Unit *uresp1 = res->lsu + D2U(set->digits);
        res->bits = src->bits;
        for (; ur < uresp1; ur++, ul++) {
            *ur = *ul;
        }
